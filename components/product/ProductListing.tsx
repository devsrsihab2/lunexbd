...
export function ProductListing({ initialProducts, initialQuery }: ProductListingProps) {
  const baseQuery = useMemo<ProductQuery>(() => cleanQuery({
  ...
  const [products, setProducts] = useState<Product[]>(initialProducts.success ? initialProducts.data : []);
  const [page, setPage] = useState(initialProducts.pagination?.page || 1);
  const [totalProducts, setTotalProducts] = useState(initialProducts.pagination?.total || products.length);
  const [totalPages, setTotalPages] = useState(initialProducts.pagination?.totalPages || 1);
  const [message, setMessage] = useState(initialProducts.success ? "" : initialProducts.message || "Products unavailable.");
  const [loadingMode, setLoadingMode] = useState<"filter" | "more" | null>(null);
  ...
  const loadProducts = useCallback((query: ProductQuery, nextPage = 1, append = false) => {
  ...
  function handleReset() {
    setFilters(cleanQuery({ category: initialQuery.category || "", sort: "latest" }));
  }
  ...
  return (
    <>
      <section className={styles.toolbar}>
        <div>
          <h2>{filters.search ? "Matching Products" : filters.category ? "Category Products" : "All Products"}</h2>
          <p>Filter by keyword, price and availability to find the right product faster.</p>
        </div>
      ...
      <div className={styles.layout}>
        <aside className={styles.sidebar}>
          <ProductFilters values={filters} onChange={handleFiltersChange} onReset={handleReset} />
        </aside>
        <section className={styles.content}>
          <div className={styles.resultBar}>
            <span>{filters.search ? `Search: ${filters.search}` : filters.category ? `Category: ${filters.category.replaceAll("-", " ")}` : "All products"}</span>
            <span>{filters.sort ? `Sorted by ${filters.sort.replace("_", " ")}` : "View All"}</span>
          </div>
              ...
              <div className={styles.loadMoreWrap}>
                {hasMore ? (
                  <button type="button" className={styles.loadMore} onClick={handleLoadMore} disabled={isLoadingMore}>
                    {isLoadingMore ? <><span className={styles.spinner} aria-hidden="true" /> Loading...</> : "Load more"}
                  </button>
                ) : products.length ? (
                  <p className={styles.endText}>You have viewed all products.</p>
                ) : null}
              </div>
            </>
          )}
